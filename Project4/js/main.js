...
//preload images
PIXI.loader.add(["images/duckwalk5.png", "images/explosions.png"]).on("progress", e=>{console.log(`progress=${e.progress}`)}).load(setup);
...
//game vaiables
let startScene;
let gameScene,duck,scoreLabel,lifeLabel,shootSound,hitSound,fireballSound;
let gameOverScene;
...
function gameLoop(){
    ...
    //keep duck on screen with clamp()
    let w2 = duck.width/2;
    let h2 = duck.height/2;
    duck.x = clamp(newX,0+w2,sceneWidth-w2);
    duck.y = clamp(newY,0+h2,sceneHeight-h2);
...
function fireBullet(e){
    ...
    let b = new Bullet(0xFFFFFF, duck.x, duck.y);
    bullets.push(b);
    ...
    //Triple shot logic
    if( levelNum > 1 ){
    let c = new Bullet(0xFFFFFF, duck.x - 15, duck.y);
    bullets.push(c);
    gameScene.addChild(c);
    let d = new Bullet(0xFFFFFF, duck.x + 15, duck.y);
    bullets.push(d);
